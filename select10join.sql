...
2) 학생명, 학생학과번호, 교수의 학과명, 교수명, 교수 학과번호
select s.name, s.deptno1, d.dname, p.name, p.deptno
from student s, professor p, department d
where s.profno = p.profno
...
select s.name, s.deptno1, ds.dname, p.name, p.deptno, dp.dname
from student s inner join professor p
on s.profno = p.profno   -- 학생 <--> 지도교수     (profno 기준)
inner join department ds
on s.deptno1 = ds.deptno   -- 학생 <--> 학과정보     (deptno1 | deptno)
inner join department dp
on p.deptno = dp.deptno;   -- 교수 <--> 학과정보     (deptno 기준)
...
point 점수 g_strat    g_end 사이에 들어가면 해당 사은품 수령
